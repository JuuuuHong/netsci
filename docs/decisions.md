...
- 이유:
  - 따옴표 없는 전문 검색은 단어가 흩어져 나오는 논문까지 포함해 코퍼스에 다른 분야가 섞였다(리튬 26,685 → 4,438, RAG 14,166 → 964)
...
- 관찰:
  - 따옴표로 좁혀도 리튬 topics `gaps` 상위 10쌍 중 9쌍은 "배터리 핵심 토픽 × 주변 토픽" 이다. 주변 토픽 편수만 줄었다(광촉매 1,296 → 39). 논문당 토픽 3개 제한이 원인이라 코퍼스를 좁히는 것만으로는 풀리지 않는다
  ...
  - RAG PageRank 1위 DPR(내부 인용 8회)은 RAG 원 논문 → DPR → FiD → {DPR, RAG 원 논문} 순환 때문이다. 세 논문의 코퍼스 내 나가는 간선이 1·1·2개뿐이라 점수가 고리에 쌓인다
  - RAG 원 논문(arXiv 2005.11401) 기록의 제목이 틀려 있다. 앞서 본 Chain-of-Thought 기록과 같은 arXiv DOI(`10.48550`) 계열이다
  - 리튬 수집 첫 실행이 응답 본문 읽기 타임아웃으로 끝났다. 전송 오류 미재시도(3차 리뷰에서 보류한 항목)가 실제로 발생했고, 재실행으로 캐시에서 이어받았다. 빈도가 낮고 재실행으로 복구되므로 README 한계에 기록만 한다
