...
> ⚠️ **아래 결과는 이전 버전(2026-09-16, concepts 분류 · 초록·토픽 수집 전) 실행 결과입니다.**
> 현재 버전은 기본 분류가 topics 이고 `stats` 등의 열 구성이 달라, 같은 명령의 출력 형식이 아래와 다릅니다.
> topics 기준으로 리튬 금속 음극 조건 전량(26,604편)을 다시 수집하고 있으며, 끝나면 이 절을 새 결과로 교체합니다.
...
아래 "한계" 에서 보듯 이 중 상당수는 실제로 함께 다뤄지지 않아서가 아니라 개념 태깅의 부산물로 보입니다.

**판단 기준 — 무엇을 세고 무엇을 세지 않는가**
- 세는 것: 수집한 코퍼스 안에서 두 개념 **태그**가 같은 논문에 붙은 횟수와, 두 개념이 무관할 때의 기대 횟수
- 세지 않는 것: 실제로 함께 연구됐는지. 코퍼스 밖 논문, 태그가 빠진 논문, 다른 표기는 반영되지 않는다
- `expected ≥ 3` 하한: 두 개념이 무관하고 공존 수가 대략 포아송 분포를 따른다면, 기대값 λ 에서 공존이 0 일 확률은 e^(−λ) 이다.
  λ = 3 이면 약 5%, λ = 9.44 이면 약 0.008% 다. 즉 하한 3 은 "우연히 0 일 확률 약 5% 이하" 와 비슷한 수준이다.
  다만 개념끼리 독립이 아니고 수천 쌍을 동시에 보므로 엄밀한 통계 검정은 아니다 (순위를 매기는 기준일 뿐이다)
...
- **`lift` 가 낮다고 연구 가치가 있다는 뜻은 아닙니다 — 단지 후보일 뿐입니다.** 예를 들어 3위 `Lithium metal × X-ray photoelectron spectroscopy`
  는 필터를 적용하기 전 원자료에서도 두 개념이 한 논문에 함께 붙은 적이 없습니다. 태그 기준 공존 0 은 "함께 다뤄지지 않았다" 가 아니라
